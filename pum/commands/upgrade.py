...
class Upgrade(PUMCommand):

    def add_cli_parser(subparsers):
        parser = subparsers.add_parser('upgrade', help='upgrade db')
        parser.add_argument(
        ...
        parser.set_defaults(func=Upgrade.run)
...
class Upgrader:
    ...
    def __get_delta_files(self):
        ...
        return sorted(
            deltas,
            key=lambda x: (x.get_version(), x.get_type(), x.get_name())
        )
    ...
    def show_info(self):
        """Print info about found delta files and about already made
        upgrades"""
    ...
    def __print_table(table):
        ...
        print("| " + " | ".join(
            "{:{}}".format(
                '-' * col_width[i], col_width[i]) for i, x in enumerate(
                    table[0])) + " |")
        for line in table[1:]:
            print("| " + " | ".join("{:{}}".format(x, col_width[i])
